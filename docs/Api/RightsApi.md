...
```php
rightsGet($type, $baker, $cycle, $level, $slots, $round, $status, $select, $sort, $offset, $limit): \Tzkt\Model\BakingRight[]
```
...
$type = new \Tzkt\Model\\Tzkt\Model\RightsGetCountTypeParameter(); // \Tzkt\Model\RightsGetCountTypeParameter | Filters rights by type (`baking`, `attestation`)
...
try {
    $result = $apiInstance->rightsGet($type, $baker, $cycle, $level, $slots, $round, $status, $select, $sort, $offset, $limit);
    print_r($result);
} catch (Exception $e) {
...
| ------------- | ------------- | ------------- | ------------- |
| **type** | [**\Tzkt\Model\RightsGetCountTypeParameter**](../Model/.md)| Filters rights by type (&#x60;baking&#x60;, &#x60;attestation&#x60;) | [optional] |
...
```php
rightsGetCount($type, $baker, $cycle, $level, $slots, $round, $status): int
```
...
try {
    $result = $apiInstance->rightsGetCount($type, $baker, $cycle, $level, $slots, $round, $status);
    print_r($result);
} catch (Exception $e) {
